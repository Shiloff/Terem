...
namespace Business.DataAccess.Private.Repository.Specific
{
    public class EFMessageRepository : IMessageRepository
    {
        ...
        {
            get { return context.MessagesLast; }
        }
        ...
        public ICollection<Message> GetDialog(long ProfileIdFrom, long ProfileIdTo, int count = 100)
        {
            var result = context.Messages
                .Where(
                    m =>
                        (m.ProfileIdFrom == ProfileIdFrom && m.ProfileIdTo == ProfileIdTo) ||
                        (m.ProfileIdTo == ProfileIdFrom && m.ProfileIdFrom == ProfileIdTo))
        ...
        public List<Profile> GetMyDialogs(long ProfileIdFrom)
        {
            ...
            var FromMe = context.MessagesLast
                ...
                .OrderByDescending(m => m.DT)
                .Select(m => new {Profile = m.ProfileTo, m.DT}).ToList();
            var FromTo = context.MessagesLast
                ...
                .OrderByDescending(m => m.DT)
                .Select(m => new {Profile = m.ProfileFrom, m.DT}).ToList();
        ...
        {
            var message = Messages.Where(m => m.MessageId == messageId && m.ProfileIdTo == profileId).FirstOrDefault();
            if (message != null)
            {
                message.Read = true;
            }
